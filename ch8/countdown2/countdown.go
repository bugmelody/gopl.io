...
func main() {
	...
	如果采用普通的receive操作,不论先从tick或者是 abort channel 中接收,都会造成阻塞,因此需要使用 select.
	*/
	/**
	...
	*/
	...
	/**
	 The time.After function immediately returns a channel, and starts a new goroutine that sends a
	 single value on that channel after the specified time. The select statement below waits until the
	 first of two events arrives, either an abort event or the event indicating that 10 seconds have
	 elapsed. If 10 seconds go by with no abort, the launch proceeds. 
	 */
	
	// 下面代码的意思:
	// 要么等待10秒后进行发射, 要么10秒内收到 about 信号取消发射.
	select {
